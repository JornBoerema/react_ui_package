...
import { daysInMonth, formatDateFromString, getFirstWeekdayOfMonth, GetShortDayNameByIndex } from "../../utils/DateUtils";
	...
	return (
		<div className="flex flex-col items-center w-72 p-2 rounded-lg bg-white border border-gray-300">
			<div className="flex justify-between items-center w-full">
				<Button leadingicon={<ChevronLeft title={undefined} titleId={undefined} />} variant={ButtonVariant.tertiaryGray} size={ButtonSize.square} onClick={handlePreviousMonthClick} />
				<p className="text-md font-semibold text-gray-700">{formatDateFromString("DD MMM YYYY", props.value, locale)}</p>
				<Button leadingicon={<ChevronRight title={undefined} titleId={undefined} />} variant={ButtonVariant.tertiaryGray} size={ButtonSize.square} onClick={handleNextMonthClick} />
			</div>
	...
	return (
		<div className={`relative ${props.fullwidth && "w-full"}`}>
			...
			<span className="absolute h-full top-0 right-2 flex items-center justify-center text-gray-500 pointer-events-none duration-200 ease-in-out">
				<Calendar width={20} height={20} title={undefined} titleId={undefined} />
			</span>
